...
function move(chosenPawn, diceRoll) {
    ...
    //Moving from any other space
        ...
        if (isStart(endSpace.getSpace())) {
            ...
            let newScoreArray = scoreToArray();

            let scoreMsg = Messages.sendScore;
            scoreMsg.data = newScoreArray;
            socket.send(JSON.stringify(scoreMsg));
        }

        pawn.appendTo('#'+endSpace.getSpace());
        $('#'+selectedPawn).removeClass('clicked');
    }
}
